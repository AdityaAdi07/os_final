...
void applyGrayscale(std::vector<Pixel>& pixels, int start, int end, std::vector<double>& threadTimes, int threadIndex) {
    ...
    threadTimes[threadIndex] = elapsedTime.count();
}
...
bool processBMP(const std::string& inputFilename, const std::string& outputFilename) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        threads.emplace_back(applyGrayscale, std::ref(pixels), start, end, std::ref(threadTimes), i);
    }
    ...
    for (int i = 0; i < numThreads; ++i) {
        std::cout << "Thread " << i << " processing time: " << threadTimes[i] << " seconds" << std::endl;
    }
    ...
    // Write data to CSV for visualization
    std::ofstream csvFile("image_data.csv");
    csvFile << "x,y,brightness\n";
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            const Pixel& pixel = pixels[y * width + x];
            int brightness = static_cast<int>(0.3 * pixel.r + 0.59 * pixel.g + 0.11 * pixel.b);
            csvFile << x << "," << y << "," << brightness << "\n";
        }
    }
    csvFile.close();
